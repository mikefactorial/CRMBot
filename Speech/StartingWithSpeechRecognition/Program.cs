...
namespace StartingWithSpeechRecognition
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Hello! I am CRMinator. What would you like me to do?");
        ...
        static void speechRecognitionWithDictationGrammar_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            Console.WriteLine("Okay. Let me try to : " + e.Result.Text + ". Give me a moment...");

            //API Call
            CallBotEndpoint(e.Result.Text);
        }

        static void CallBotEndpoint(string text)
        {
            /*
            BotRequest botRequest = new BotRequest();
            var client = new RestClient("http://localhost:3978");
            var request = new RestRequest("/api/messages", Method.POST);
            request.AddParameter()
            // automatically deserialize result
            IRestResponse <BotResponse> response = client.Execute<BotResponse>(request);
            */
        }
        #endregion
